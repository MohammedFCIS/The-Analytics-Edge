...
climate_testing <- subset(climate, Year >= 2007)
climate_model <- lm(Temp ~ MEI + CO2 + CH4 + N2O + CFC.11 + CFC.12+ TSI + Aerosols, data = climate_training)
...
* Enter the coefficient of `N2O` in this reduced model:`2.532e-0`
* Enter the model `R^2^`:`0.7261`

```{r}
climate_model <- lm(Temp ~ MEI + N2O + TSI + Aerosols, data = climate_training)
summary(climate_model)
```
...
Enter the R2 value of the model produced by the step function: `0.7534`
Which of the following variable(s) were eliminated from the full model by the step function? Select all that apply.
```{r}
climateLM <- lm(Temp ~., data = climate_training)
climateLMBest <- step(climateLM)
summary(climateLMBest)
```
...
```{r}
fit <- predict(climateLMBest, newdata = climate_testing)
SSE <- sum((fit - climate_testing$Temp)^2)
